...
\section{Ab Initio Many Body Quantum Mechanics}
 ...
 The non-relatavistic physics of electron and nuclei in a stationary state is described by the time-independent Schr{\"o}dinger equation (SE)
  ...
  %The approximation allows one to neglect the kinetic energy of nuclei and consider nuclear-nuclear repulsion as a constant. What is left is the N-body electronic Hamiltonian
  ...
  \subsection{Hartree-Fock}
    %Though it may not be possible to analytically solve the SE for an arbitrary number of electrons, it is able to exactly solve the differential for one electron. The idea of solving the SE in a basis of N-tuple single electron functions is the fundamental basis for an approximate solution, known as Hartree-Fock (HF) theory\cite{Hartree1928b,SzaboAttila1982,Sherrill2000}. 
    ...
    The best approximate solution can be found by variational minimization, which provides the HF equations:
    %The best approximate solution can be found by variationally minimizing the Rayleigh quotient
    %  \begin{equation}
    %  E_{0} = \frac{\bra{\Psi_{0}} \hat{H}_{\text{elec}} \ket{\Psi_{0}}}{\selfolap{\Psi_{0}}}
    %  \end{equation}
    ...
    %      \end{cases}
    %  \end{equation}
    %and where the bracket notation implies the Hilbert space inner product. ELements of inner product $\bra{\Psi_{o}} \hat{H}_{elec} \ket{\Psi_{0}}$ form a set of eigenvalue integro-differential equation over all space, the HF equations, with each element defined as 
    ...
    The last two terms in \cref{Fock_op} represent the potential energy of the electron-electrons interaction. $\hat{J}_j$ represents the Coulombic repulsion between two electrons:
      ...
      \end{equation}
    $\hat{K}_j$, the exchange term, does not have a classical interpretation% as the previous terms do and 
    ...
    %The solutions of the Hydrogen atom define one electron orbital functions, $\{\chi_k\}$, exactly as Slater type orbitals. 
    Exact one one particle functions of the SE are described as  Slater type orbitals
  ...
  \subsection{Electronic Correlation Methods}
    ...
    %properties of fermions and Coulombic repulsion between electronic charges. 
    ...
    %by definition this is an uncorrelated description as it does not consider electron-electron repulsion explicitly but as %electron repulsion with an average electron cloud charge. 
    ...
    \subsubsection{Many Body Perturbation Theory}
      ...
      %Though there are many formulations of MBPT, two of the most well known being Rayleigh-Schr{\"o}dinger\cite{Lindgren1974} and M{\/o}ller-Plesset\cite{Moller1934,Raghavachari1989}, all methods are developed from the same principles. 
      ...
      %One can then expand the wavefunction and energy
       % \begin{equation}
       ...
       Perturbative expansion of the \cref{eigenvalue} with \cref{H_pert} provides the following expression:
    ...
    \subsubsection{Configuration Interaction}
      ...
      %CI methods diagonalize the N-electron hamiltonian in terms of $\Psi_{exact}$. This can be achieved by expanding $\Psi_{corr}$ as a linear combination of all possible N-tuply excited slater determinants%slater determinants constructed from N electrons and 2K spin orbitals, $\{\chi_k\}$ 
      ...
      %provides an equality to $\Psi_{exact}$. 
      ...
      %by Slater-Condon rules one finds
        ...
        \end{equation}
      To resolve $C^{ij}_{ab}$ it is necessary to solve the expression:%project excited determinants onto \cref{CI_E}
      ...
      The CISD formulation has computational scaling of $\mathcal{O}(N^6)$ and storage requirement of $\mathcal{O}(N^4)$. Any truncation to the full-CI wavefunction eliminates the size-consistency and size-extensiveness of the method. To be size-extensive a method must scale linearly with number of electrons, N.%Additionally, truncated CI is not size-extensive, meaning the energy calculated by truncated CI methods do not scale linearly with number of electrons, N; CI is variational.
    ...
    \subsubsection{Coupled Cluster Theory}
      ...
      the $n$th order cluster operator has the form 
        ...
        \end{equation}
      where $a_i$ and $a^\dagger_a$ are second-quantization operators: $a_i$ deletes an orbital $\phi_i$ and $a^\dagger_a$ inserts an orbital $\phi_a$ from the determinant which the operators act upon, $\chi_l$ %Therefore the cluster operators replace n orbitals, $\phi_i$, $\phi_j, \dots$ with $\phi_a$, $\phi_b,\dots$ respectively.
      ...
      %though the number of terms and the computational scaling factor quickly becomes unmanageable. 
      %To deal with this problem the cluster operator, $\hat{T}$ is truncated. The derivation to follow will restrict the problem to the single and double cluster operators (CCSD)
        \begin{equation}
      ...
      The Hamiltonian is projected onto by $e^{-\hat{T}}$ allowing one to simplify the energy expression using the Cambell-Baker-Hausdorff formula\cite{Crawford2000} and find%then provide computationally useful equations using the following equations 
  ...
  \subsection{Explicitly Correlated Methods}
    ...
    Within quantum chemistries most accurate approximations one finds basis set error as the largest contribution to total error. %The first place this error comes from is in the expression of one electron wavefunctions. 
    %to achieve high accuracy in calculations one must enlist large basis sets and $\zeta$. 
    In the wavefunction framework established above basis set error contributions are slow to converge and fails to fully recreate the electron cusp condition. Extremely accurate calculations therefore requiring large basis sets. %Figure 1 shows the electron probability on a cross section of a sphere. Because 
    %To exemplify, 
    HF theory considers an average electronic field, i.e. electron probability relative to other electrons is constant. In truth electron-electron probability is a function of distance and angle on a sphere, see figure 1. %there is zero probability of electron one at the point of electron two and higher probaility that electron one will be found $\pi$ radians away from electron two. 
     %Corrections to HF theory are known as Coulombic correlation. 
    \begin{figure}[H]
      \centering
    ...
    To accurately calculate the Coulombic correlation energy it is necessary to generate a new wavefunction expression that is explicitly dependent on inter-electronic distances. Kato's cusp condition\cite{Kato1957} comes from realizing there exists a first-order derivative discontinuity at the Coulombic type singularity\cite{Hattig2012} which leads to the coalescence condition 
      \begin{equation}
        ...
        \end{equation}
    Using this condition, one can construct the new R12 wavefunction as: %the new form of the exact wavefunction becomes:
      %\begin{equation}
      %  \Psi_{exact}(r_1, r_2 , \dots) = \Phi(r_{12})\Psi(R_{12})
      %\end{equation}
    %Where $R_{12} \equiv \frac{r_1 + r_2}{2}$ and $r_{12} \equiv r_1 - r_2$ and the form of $\Phi{r_{12}}$ is chosen such that it satisfies the coalescence condition 
      %\begin{equation}\label{R12_func}
       % \Phi(r_{12}) = R(r_{12})\Theta(\Omega_{12})
      %\end{equation}
    %The position space representation of \cref{R12_func}, can be solved through seperation of variables. 
    %The angular function, $\Theta(\Omega_{12})$, is represented by the spherical harmonics, $Y_{lm}$, and the radial function, $R(r_{12})$ is determined using approximate solutions to the two-electron radial SE
      %\begin{equation}
    ...
    %In the first attempts to create a wavefunction that was explicitly dependent on inter-electronic distances by Hartree \cite{Hartree 1928} and Hylleras\cite{Hylleras 1929}, the cusp condition was unknown and therefore the attempts could not effectively create functions that could be efficiently calculated. 
    ...
    \subsubsection{Explicitly Correlated MP2-R12 method}
      The difference between MP2 and MP2-R12 methods is in the definition of the first order wavefunction. %In standard MP2 theory the first order wavefunction is expressed as equation (32). 
      ...
      %Doubly excited coefficients, t, can be resolved by minimizing a Hylleraas functional of the MP2 energy 
      %The first order wavefunction can be resolved by minimizing Hylleraas functional for the MP2 energy
       % \begin{equation}
        %  H^{(2)} (\Phi^{(1)}) = \bra{\Phi^{(1)}} \hat{H}_0 - E_0 \ket{\Phi^{(1)}} + 2 \bra{\Phi^{(1)}}\hat{H}\ket{\Phi^{(0)}}
        %\end{equation}
    ...
    \subsubsection{Explicitly Correlated CC-R12 method}
      %It is necessary to apply R12 methods to CC in order to achieve the theories full potential because MP2-R12 has limited chemical framework\cite{Kong2012}. 
      ...
      %with direct computation of an intermediate each iteration.\cite{Kong2012} 